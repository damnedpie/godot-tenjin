...
public class GodotTenjin extends GodotPlugin {
	...
	private TenjinSDK getTenjinInstance() {
		return TenjinSDK.getInstance(getActivity(), apiKey);
	}
	...
	public void initialize(final String _apiKey, final String _deepLinkUri, final int appstoreType) {
		apiKey = _apiKey;
		TenjinSDK instance = TenjinSDK.getInstance(getActivity(), apiKey);
	...
	public String getGAID() {
		Info adInfo;
		try {
			adInfo = AdvertisingIdClient.getAdvertisingIdInfo(getActivity());
			return adInfo.getId();
		} catch (Exception e) {
